...
- `tests/integration/test_web_search_tool.py` — инструмент `web_search` (OVE-37) на **живом
  ...
  `SEARCH_UNAVAILABLE_TEXT`, а поисковик, который принял соединение и замолчал (запрос
  `stalled`), — таймаут (OVE-39): в ошибке адрес поисковика и **нет текста запроса**.
  Зависание уже **после** загрузки выдачи (OVE-54) —
  `test_a_results_page_that_stops_answering_mid_read_times_out_instead_of_hanging`: запрос
  `refresh-to-stall` отдаёт страницу с `<meta refresh>` на молчащий адрес, бюджет чтения
  (`EXTRACTION_TIMEOUT_MS`) подменяется на 500 мс, вызов обёрнут в `asyncio.timeout`, чтобы
  регрессия была красным тестом, а не зависшим прогоном. Проверяется, что ошибка —
  `extraction_timeout_text()` с числом секунд, в ней нет ни текста запроса, ни адреса, а
  содержимого страницы-заглушки нет во всём `ToolResult`; `summary` тот же, что у таймаута
  навигации, а страница закрыта. Проброс `conversation_id` до `browser_context()` проверяется
...
- `tests/integration/test_open_page_tool.py` — инструмент `open_page` (OVE-38) на **живом
  ...
  `asyncio.timeout`, чтобы регрессия была красным тестом, а не зависшим прогоном, и
  проверяется, что в ошибке есть адрес и число секунд, `summary` без адреса, а страница
  закрыта. Там же файлы, которые браузер скачал бы (`application/pdf` и
  `Content-Disposition: attachment` при `text/html`), дают `NOT_HTML_TEXT`, а
  `playwright.Error` при извлечении (подмена `extract_page`) — `PAGE_CHANGED_TEXT`.
  Естественную версию последнего (страница, редиректящая себя сразу после DCL) в тест не
  берём: воспроизводится в 20–40% прогонов, то есть флакует. Страницы-заглушки с
  `<meta refresh>` (OVE-53) — там же, на маршруте фейка `/refresh?content=...`: переход до
  цели по каждой форме записи и по заголовку `Refresh` повторяется по 20 раз в одном тесте
  (старый код краснел на первой же итерации), авто-обновление себя возвращается без ожидания,
  далёкий refresh даёт `refresh_url`, а цель `404`, PDF, молчащий сервер, слишком длинная
  цепочка и внутренняя сеть — свои отличимые ошибки. Разбор `content` — в
  `tests/unit/test_open_page.py`;
- `tests/unit/` — юнит-тесты бизнес-логики: конфиг, LLM-клиенты и фабрика, контракт
  ...
  `browser`. Отдельно `test_browser_page.py` (OVE-39): `browser_page()` на фейках — страница,
  чей `close()` не возвращается, не подвешивает вызов (обёртка в `asyncio.timeout`), сносит
  сессию разговора, и следующий вызов получает свежий контекст; ошибка самого вызова
  переживает зависшее закрытие; упавший `close()` только логируется и сессию не трогает.
  `test_browser_egress.py` держит и то, что контексты создаются с `accept_downloads=False`.
  Там же — восстановление после падения браузера **между двумя арендами одного

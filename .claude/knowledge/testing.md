...
- `tests/unit/` — юнит-тесты бизнес-логики: конфиг, LLM-клиенты и фабрика, контракт
  ...
  ответ, записанный до реконнекта; программный вход в запись — в
  `test_voice_wake_word.py`, там же, где обычный путь через wake word: те же инварианты
  `generation`, гейта соединения и зафиксированной эпохи) и изоляция от CI
  (`test_voice_ci_isolation.py`), а по
